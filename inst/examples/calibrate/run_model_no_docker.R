...
#
# Variant of run_model.R without docker: the run script is executed directly
# with bash, so Atlantis must be available in the same environment as this R
# session.
#
# Prerequisites:
# - the Atlantis binary `atlantisMerged` on the PATH (the command called by
#   the run script template inst/run_scripts/script.sh),
...
#
# To drive this variant from launch.R, source this file there instead of
# run_model.R and drop "docker_user" from setup_cluster_nodes(): it does not
# exist here.

library(atlantio)

if (Sys.which("atlantisMerged") == "") {
  cli::cli_abort("{.code atlantisMerged} not found on the PATH.")
}
...
#' the script runs with `run_dir` as its working directory.
#'
...
#' the input files and run script, run the model directly with bash and
#' compute the objective score. Each run leaves a folder under `results_dir`
#' holding the tracer time series the score is derived from
#' (`time_series.rds`) and the parameter values with the score
#' (`parameters.rds`, calibration scale, map to model values with
#' `transform_parameter_value()`); every other run file is removed, unless
#' `keep_files = TRUE`.
#'
